...
import numpy as np
# ~ Local ~
from marchhare.marchhare import concat_arr , enumerate_reverse
from marchhare.MathKit import eq , left_divide , ver , verd
from marchhare.Vector import np_dot , vec_unit , vec_mag, vec_random_range
from marchhare.VectorMath.Vector3D import x_rot , x_trn , y_rot , z_rot , z_trn , rot_matx_ang_axs , ang_axs_from_rot_matx , homogeneous_Z , \
                                          homog_ang_axs , apply_homog , homog_xform , skew_sym_cross 
...
class LinkSpatial:
    ...
    def __init__( self , pName , pPitch , E_xfrm = None , r_xfrm = None ):
        ...
        self.xform = homog_xform( E_xfrm , r_xfrm ) # ----- Homogeneous coordinate transform -> position of this joint in the parent frame
...
#        [ XJ , s_i ] = joint_xform( link.pitch , link.q ) # (FS: 'jcalc') calc the joint transformation and freedom (selection) matrix
    ...
    for lnkDex , link in enumerate_reverse( model.links ): # For every link in the model (reverse order) , do
